...
void BuddyAllocator::GetMemMap(Region regions[], size_t regionCount) {
...
bool BuddyAllocator::InitializeImpl(RegionBlocks regions[], size_t regionCount)
{
    ...
    {
        dbg_printf("no free space! needed: %llu regionCount: %d\n", m_BitmapSize, regionCount);
        return false;
    }
...
void BuddyAllocator::MarkRegion(void* basePtr, size_t sizeBytes, bool isUsed)
{
    ...
    size_t size = DivRoundUp((long long unsigned int)sizeBytes, m_SmallBlockSize);
...
void BuddyAllocator::MarkBlocks(uint64_t block, size_t count, bool isUsed)
{
    ...
    {
        block /= 2;
        count = DivRoundUp((unsigned long int)count, 2UL);
...
// for statistics
/*
RegionType BuddyAllocator::GetState(void* address)
{
